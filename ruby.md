...
### Khái niệm về `Block/lambda/proc`
...
- Giống như method, ta có thể truyển đối số cho `yield`
...
- `Lambda` là hàm nặc danh (khá giống `javascript`)
- Thuộc `Proc` object, là một `special Proc object`
...
- Là đối tượng đại diện cho `block`, là `block` được đặt tên
- Có thể lưu trữ `proc` trong 1 biến và sau đó truyền nó tới bất kỳ method nào gọi nó
...
- `Proc` và `Lambda` đều là closure trong ruby, là phương pháp nhóm gộp các đoạn code lại để thực thi
...
- `Block` không phải đối tượng, còn `Proc` và `Lambda` thì có
- `Proc` có hành vi giống với `Block` còn `Lambda` thì giống method

#### Proc và lambda

2 điểm khác biệt chính:

- Xử lý tham số truyền vào: `Proc` không quan tâm đến tham số truyền vào, `lambda` thì có
...
t = lambda { |x,y| puts "I don't care about arguments!" }
t.call
# => ArgumentError: wrong number of arguments (given 0, expected 2)
```

- Cách trả về `return`:
  - `Lambda` sẽ trả về giá trị của bản thân nó như một `method` bth, trong khi `Proc` chỉ trả về cho ngữ cảnh hiện tại
  - Nói cách khác, `Proc` sẽ return ngay sau khi thực hiện xong, còn `lambda` thì vẫn tiếp tục chạy hết hàm sau khi gọi
  - VD1:
